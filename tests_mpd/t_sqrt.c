...
void check_sqrt(const char *a, unsigned int prec_a,
  ...
  if (print==1) {
    ...
    gmp_printf(" sqrt ( %Zd x 10 ^ %d ) \t=\t",MPD_MANT(A),MPD_EXP(A));
    gmp_printf("%Zd x 10 ^ %d\n",MPD_MANT(C),MPD_EXP(C));
  }
  
  mpd_sqrt(res,A,rnd_mode);
...
#define check53_sqrt(x, r, z, c) check_sqrt(x, 16, r, 16, z, c)

void t_sqrt(int print){
